...
1. Token blocking: explode each record's normalized "core" name tokens and
   ...
   (appear in a large fraction of records, e.g. generic words like "market")
   are dropped from the join key set — they would blow up the candidate space
   without adding discriminative signal — but every record keeps at least its
   rarest token so nothing is blocked out entirely.
...
MAX_TOKEN_DOC_FREQ = 0.003   # drop join tokens present in >0.3% of a source's records
...
def _frequent_tokens(ex: pd.DataFrame, n_records: int) -> set:
    if n_records == 0:
        return set()
    doc_freq = ex.groupby("token")[ex.columns[0]].nunique()
    thresh = max(5, int(n_records * MAX_TOKEN_DOC_FREQ))
    return set(doc_freq[doc_freq > thresh].index)
...
def build_candidates(s1_df: pd.DataFrame, other_df: pd.DataFrame, k: int = TOP_K_CANDIDATES
    ...
    oth_ex = _explode_tokens(other_df.assign(entity_id=other_df["entity_id"]), "entity_id", "name_core_tokens")

    drop_tokens = _frequent_tokens(oth_ex, len(other_df)) | _frequent_tokens(s1_ex, len(s1_df))
    ...
    postal_pairs = s1_df[s1_df["postal_code"] != ""][["entity_id", "postal_code"]].merge(
        other_df[other_df["postal_code"] != ""][["entity_id", "postal_code"]],
    ...
    prefix_pairs = s1_df[s1_df["name_prefix"] != ""][["entity_id", "name_prefix"]].merge(
        other_df[other_df["name_prefix"] != ""][["entity_id", "name_prefix"]],
    ...
    sdx_pairs = s1_df[s1_df["soundex1"] != ""][["entity_id", "soundex1"]].merge(
        other_df[other_df["soundex1"] != ""][["entity_id", "soundex1"]],
