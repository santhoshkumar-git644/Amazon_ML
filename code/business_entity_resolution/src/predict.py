"""Run the trained GPU pipeline on the test set and write the two submission files:
...
def main():
    ...
    booster = xgb.Booster()
    booster.load_model(os.path.join(args.model_dir, "matcher.json"))

    log("loading test files (GPU CSV parse) ...")
    ...
    all_s1_ids = s1["entity_id"].to_pandas().tolist()
    ...
    log("blocking (candidate generation, GPU) ...")
    t0 = time.time()
    ...
    candidate_by_s1 = pairs.to_pandas().groupby("s1_id")["cand_id"].apply(list).to_dict()
    ...
    log("computing features (GPU) ...")
    t0 = time.time()
    feats = pipeline.compute_features_for_pairs(pairs, s1n, s2n, s3n)
    log(f"  -> {feats.shape} in {time.time() - t0:.1f}s")

    log("scoring with model (GPU) ...")
    dtest = xgb.DMatrix(feats[feature_columns])
    probs = booster.predict(dtest)
    probs = cp.asarray(probs).get() if hasattr(probs, "get") else np.asarray(probs)
    feats["prob"] = probs

    accepted = feats[feats["prob"] >= threshold]
    matched_by_s1 = accepted.to_pandas().groupby("s1_id")["cand_id"].apply(list).to_dict()
